...
public class Tone2 {
	...
	private static volatile Boolean mainTurn = true;
    ...
    void playSong(List<BellNote> song) throws LineUnavailableException {
        try (final SourceDataLine line = AudioSystem.getSourceDataLine(af)) {
            ...
            for (BellNote bn: song) {
                ...
                playNote(line, bn);
            	while(!mainTurn) {
//            		System.out.println(mainTurn);System.out.println(Thread.currentThread().getName());
//            		System.out.println(mainTurn);
            	} //Wait for appropriate thread to play
                System.out.println(bn.note+" "+bn.length + Thread.currentThread());
            }
            line.drain();
        }
    }
	...
	private class Task implements Runnable{
		...
		public void run() {	
			while(true) {
				...
				if(cNote.equals(this.note) && !mainTurn) {
					System.out.println(Thread.currentThread().getName() + " plays " + current_note.note.name());
